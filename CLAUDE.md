...
- IA: **Home "This Check"** (period picker, status hero, Check my paycheck → verdict, breakdown, what-if) · **Shifts** (cards + bottom-sheet editor, schedule scan) · **Me** (pay rules in plain rows, Advanced disclosure for all tax config, open questions, periods/data/backup, and the in-app manual `src/screens/HowTo.tsx` — "How to use this app", with a replay-tour button). First run shows a 4-screen onboarding (skippable; `settings.onboarding`).
